...
class Trainer(object):
    ...
    def train_epoch(self, num_epoch=None):
       ...
       -loop on the number of iterations in the config and call the train step
        ...
        # One Train step on the current batch
        loss, correct_labels_or_distances = self.model_wrapper.run_model_get_loss_and_results(self.data_loader.train_graphs, self.data_loader.train_labels)
    ...
    def validate(self, epoch):
        ...
        # One Train step on the current batch
        loss, correct_labels_or_distances = self.model_wrapper.run_model_get_loss_and_results(self.data_loader.val_graphs, self.data_loader.val_labels)
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        ...
        if self.is_QM9:
            val_dists = (total_correct_labels_or_distances*self.data_loader.labels_std)/self.data_loader.val_size
            ...
            val_acc = total_correct_labels_or_distances/self.data_loader.val_size
    ...
    def test(self, load_best_model=False):
        ...
        # One Train step on the current batch
        loss, correct_labels_or_distances = self.model_wrapper.run_model_get_loss_and_results(self.data_loader.test_graphs, self.data_loader.test_labels)
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        ...
        # update results from train_step func
        total_loss += loss
        total_dists += correct_labels_or_distances
